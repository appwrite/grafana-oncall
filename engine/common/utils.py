...
# Host repetition is bounded by the DNS name limit; URL paths have no nested
# quantifiers. Keep this matcher free of ambiguous nested repetition.
url_re = re.compile(
    r"""\b(?:https?://[^\s<>\[\]{}"']+|(?<!@)[a-z0-9][a-z0-9.-]{0,252}\.[a-z]{2,63}(?:/[^\s<>\[\]{}"']*)?)""",
    re.IGNORECASE,
)


def find_urls(text):
    # Match only a bounded rendering input. Linkification itself happens later.
    links = []
    for match in url_re.finditer(text[:100_000]):
        link = match.group().rstrip(".,;:!?`")
        # Keep balanced parentheses in paths, but leave Markdown delimiters out.
        excess = min(len(link) - len(link.rstrip(")")), max(0, link.count(")") - link.count("(")))
        if excess:
            link = link[:-excess]
        links.append(link)
    return links
